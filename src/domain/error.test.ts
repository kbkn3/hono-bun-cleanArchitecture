...
describe("Error", () => {
  describe("Status", () => {
    it("正しいメッセージを返す", () => {
      ...
      expect(Status.SYSTEM_ERROR.toMessage()).toBe("System Error");
  ...
  describe("ApplicationConfigurationError", () => {
    it("正しいエラーメッセージを生成する", () => {
      ...
      expect(error.message).toBe("Configuration Error: Missing env var");
    });
  ...
  describe("ApplicationStatusError", () => {
    it("正しいエラーメッセージを生成する", () => {
      ...
      expect(error.message).toBe("Application Status Error status: Illegal Data message: Invalid data");
    });
